...
class RelativePositionSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        q = (
            self.query(query)
            .view(batch_size, query_len, self.num_heads, self.d_k)
            .transpose(1, 2)
        )
        k = (
            self.key(key)
            .view(batch_size, key_len, self.num_heads, self.d_k)
            .transpose(1, 2)
        )
        v = (
            self.value(value)
            .view(batch_size, key_len, self.num_heads, self.d_v)
            .transpose(1, 2)
        )
        ...
        relative_position = (
            position_ids_q.unsqueeze(-1)
            - position_ids_k.unsqueeze(0)
            + self.max_len
            - 1
        )  # 偏移以确保索引>=0
...
class LSHSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        mask = hash_codes_q.unsqueeze(-1) != hash_codes_k.unsqueeze(
            -2
        )  # [batch, heads, query_len, key_len]
